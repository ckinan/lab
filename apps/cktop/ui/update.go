...
type snapshotMsg proc.Snapshot

func waitForSnapshot(ch <-chan proc.Snapshot) tea.Cmd {
	return func() tea.Msg {
		snap, ok := <-ch
		if !ok {
			return nil
...
func (m *Model) applySort() {
	...
	var sorted []proc.Process
	procs := proc.FilterProcesses(m.procs, m.filter.Value())
	if !m.showKThreads {
		filtered := procs[:0]
		for _, p := range procs {
			if !p.IsKthread {
	...
	switch m.sortBy {
	case SortByRSS:
		sorted = util.SortBy(procs, func(p proc.Process) int { return p.Rss }, m.sortDesc)
	case SortByCPU:
		sorted = util.SortBy(procs, func(p proc.Process) float64 { return p.CPU }, m.sortDesc)
	case SortByPID:
		sorted = util.SortBy(procs, func(p proc.Process) int { return p.Pid }, m.sortDesc)
	case SortByPPID:
		sorted = util.SortBy(procs, func(p proc.Process) int { return p.Ppid }, m.sortDesc)
	case SortByCmdLine:
		sorted = util.SortBy(procs, func(p proc.Process) string { return p.Cmdline }, m.sortDesc)
	}
...
func appendTreeRows(pid int, pByPid map[int]proc.Process, childrenByPid map[int][]int, prefix string, rows []table.Row, pids []int) ([]table.Row, []int) {
...
func buildTreeRows(procs []proc.Process, selected proc.Process) ([]table.Row, []int) {
	pByPid := make(map[int]proc.Process, len(procs))
	for _, p := range procs {
		pByPid[p.Pid] = p
	}
	childrenByPid := proc.BuildChildren(procs)
	parents := proc.BuildParents(procs, selected)
