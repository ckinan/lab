...
import (
	...
	"github.com/ckinan/lab/libs/goruntime"
...
func (c *memCache) get() proc.Memory {
	c.mu.RLock()
	defer c.mu.RUnlock()
	return c.mem
}
...
func (c *loadAvgCache) get() proc.LoadAvg {
	c.mu.RLock()
	defer c.mu.RUnlock()
	return c.val
}
...
func (c *fileNRCache) get() proc.FileNR {
	c.mu.RLock()
	defer c.mu.RUnlock()
	return c.val
}
...
func (c *sockStatCache) get() proc.SockStat {
	c.mu.RLock()
	defer c.mu.RUnlock()
	return c.val
}
...
func (c *vmStatCache) get() proc.VMStat {
	c.mu.RLock()
	defer c.mu.RUnlock()
	return c.val
}
...
type diskCache struct {
	mu   sync.RWMutex
	devs map[string]proc.DiskStat
}

func newDiskCache() *diskCache {
	return &diskCache{devs: make(map[string]proc.DiskStat)}
}
...
func (c *diskCache) getDev(device string) proc.DiskStat {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
type netDevCache struct {
	mu     sync.RWMutex
	ifaces map[string]proc.NetDev
}

func newNetDevCache() *netDevCache {
	return &netDevCache{ifaces: make(map[string]proc.NetDev)}
}
...
func (c *netDevCache) getIface(iface string) proc.NetDev {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *psiCache) get() proc.SystemPSI {
	c.mu.RLock()
	defer c.mu.RUnlock()
	return c.val
}
...
type cgroupPSICache struct {
	mu       sync.RWMutex
	services map[string]proc.ServicePSI
}

func newCgroupPSICache() *cgroupPSICache {
	return &cgroupPSICache{services: make(map[string]proc.ServicePSI)}
}
...
func (c *cgroupPSICache) refresh(r proc.CgroupPSIReader) {
	...
	c.mu.Lock()
	c.services = make(map[string]proc.ServicePSI, len(stats))
	for _, s := range stats {
		c.services[s.Unit] = s
	}
	c.mu.Unlock()
...
func (c *cgroupPSICache) getUnit(unit string) proc.ServicePSI {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *aptCache) get() proc.AptInfo {
	c.mu.RLock()
	defer c.mu.RUnlock()
	return c.val
}
...
type serviceCache struct {
	mu       sync.RWMutex
	services map[string]proc.ServiceStat
}

func newServiceCache() *serviceCache {
	return &serviceCache{services: make(map[string]proc.ServiceStat)}
}
...
func (c *serviceCache) start(interval time.Duration, r *proc.ProcServiceStatsReader) {
	...
	go func() {
		...
		for range t.C {
			...
			c.mu.Lock()
			c.services = make(map[string]proc.ServiceStat, len(stats))
			for _, s := range stats {
				c.services[s.Unit] = s
			}
			c.mu.Unlock()
...
func (c *serviceCache) getUnit(unit string) proc.ServiceStat {
	c.mu.RLock()
	defer c.mu.RUnlock()
	return c.services[unit]
}
