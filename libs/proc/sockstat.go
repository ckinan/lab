...
func (r ProcSockStatReader) ReadSockStat() (SockStat, error) {
	f, err := os.Open("/proc/net/sockstat")
	if err != nil {
		return SockStat{}, fmt.Errorf("open /proc/net/sockstat: %w", err)
	}
	defer f.Close()
...
func parseSockStat(r io.Reader) (SockStat, error) {
	...
	if err := scanner.Err(); err != nil {
		return SockStat{}, fmt.Errorf("reading /proc/net/sockstat: %w", err)
	}
	return s, nil
}
